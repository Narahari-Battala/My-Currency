...
API_KEY = "0e4be069b0625879aba9303475f60a3d"
...
def latestRates(baseCurr,date,symbols): 
    if date is not None and not symbols:
        url = "http://data.fixer.io/api/"+date+"?access_key ="+ API_KEY+"&base="+baseCurr
        print("Url Check 1 - "+str(url))
    elif date is not None and symbols:
        url = "http://data.fixer.io/api/"+date+"?access_key = "+API_KEY+"&base="+baseCurr+"&symbols="+symbols
        symbols=""
    elif symbols:
        url = "http://data.fixer.io/api/latest?access_key ="+ API_KEY+"&base="+baseCurr+"&symbols="+symbols
        print("Url Check 1 - "+str(url))
    else:
        url = "http://data.fixer.io/api/latest?access_key ="+ API_KEY+"&base="+baseCurr
    try:
